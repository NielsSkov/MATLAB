...
u = zeros(size(Yvolt));             %<--| used for 'meas[process].mat'-file
uin = [ t' u ];
...
y = (Yvolt - equVolt)*resRad-0.005; %<--| used for 'meas[process].mat'-file

scatter(t',y)

%% initialize for senstool

%the initial parameter values, 'mainest.m' looks for the 'par0'
par0 = [ B_f J_f ];

%senstool's 'mainest.m' need a 'meas[process].mat'-file containing
%mesurement data, y, input, u, and time vector, t.
save measCubli t u y

%senstool's 'mainest.m' needs a simulation function called 'sim[process].m'
process = 'Cubli';

%% >>-- RUN SENSTOOL!! :) -->>

%to use, permanently include the 'senstool'-folder in the matlab-path
run mainest.m
